    ...
    describe("Updates", function () {
        ...
        describe("dollarPriceForNamePerYear", function () {
            it("Should return a correct price", async function () {
                ...
                expect(await _oracle.dollarPriceForNamePerYear(["🐻‍❄️", "🫶", "🎃"])).to.eq(BigInt(450 * 1e18 * 69 / 100))
                expect(await _oracle.dollarPriceForNamePerYear(["🫶", "🐻‍❄️", "🎃", "💩"])).to.eq(BigInt(80 * 1e18 * 69 / 100))
                expect(await _oracle.dollarPriceForNamePerYear(["🐻‍❄️", "🫶", "🎃", "💩", "😮"])).to.eq(BigInt(25 * 1e18 * 69 / 100))
                expect(await _oracle.dollarPriceForNamePerYear(["b", "e", "r", "a", "🐻‍❄️"])).to.eq(BigInt(25 * 1e18 * 25 / 100))
        ...
        describe("price", function () {
            it("Should return correct price", async function () {
                ...
                function exponent(duration: any) {
                    return ((duration - 1) + Math.floor(((110) ** duration / 100 ** duration)))
                }

                expect(await _oracle.price(
                    ["b", "e", "r", "a"],
                    1,
                    asset
                )).to.eq(BigInt(Math.floor(80 * 1e18)))

                expect(await _oracle.price(
                    ["🦆", "🐷"],
                    1,
                    asset
                )).to.eq(BigInt(Math.floor(690 * 1e18 * 69 / 100)))
                ...
                expect(await _oracle.price(
                    ...
                    .to.eq(BigInt(Math.floor((690 * 1e18 * 69) / 100) * exponent(3)))
            })
        })
        ...
        describe("setAssetOracle", function () {
            ...
            it("Should revert if caller is not an owner", async function () {
                let assetAddress = "0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"
                let oracleAddress = _oracle.address
                await expect(_oracle.connect(_otherAccount)
